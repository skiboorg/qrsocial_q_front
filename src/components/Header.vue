  ...
  <header class="header " :class="[is_index ? 'index' : 'logged page ',$auth.loggedIn ? 'logged' : '']">
    <div class="container">
          ...
          <div class="nav-menu-item"  v-for="(link,index) in menu_links" :key="index">
            ...
            <div class="nav-menu-item__inner " :class="[
              link.need_auth ?
                   ...
                   : '']">
              <p class="no-margin text-center q-py-sm q-px-md">登录使用</p>
            </div>
      ...
      <div class="header-right_unlogged">
        <div @click="$router.push('/signup')" class="balance">充值余额</div>
        <div @click="$router.push('/signin')" class="register">注册</div>
      </div>
...
export default {
  data () {
    return {
      ...
      header_title:'',
      menu_links:[
        {name:'家',url:'index-page',need_auth:false},
        {name:'视频',url:'tiktok-page',need_auth:true},
        {name:'我们的女孩',url:'girls-page',need_auth:true},
        {name:'规则',url:'faq-rules',need_auth:true},
        {name:'联系人',url:'faq-contact',need_auth:true},
        {name:'指南',url:'faq-docs',need_auth:true},
